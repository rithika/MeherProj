...
public class HR  extends Person {
	...
	public HR(){
			...
			c2=DriverManager.getConnection("jdbc:mysql://localhost:3306/HR","root","root");
			//c3=DriverManager.getConnection("jdbc:db2:PERSON","admin","admin");
			c3=DriverManager.getConnection("jdbc:mysql://localhost:3306/PERSON","root","root");
	        }
			catch(Exception e)
			{
	    	...
	    	try{
	    		...
	    		Statement s2=c2.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			  	...
			  	while(smarks<31){
			  		...
			  		ques=ques.concat(","+str[smarks].toString()+","+str[rmarks].toString());
			  		smarks=smarks+3;
			  		rmarks=rmarks+3;
			  		qn++;
			  	}
	...
	public String Performance(char dept){
		String str=null;
		try{
			Statement s=c2.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			ResultSet rs=s.executeQuery("select QUESTIONS from QUALIFICATIONS where DEPARTMENT="+dept+" AND FLAG=1 ");
			
			//System.out.println("Result Set: "+rs.absolute(1));
			rs.first();
			str=rs.getString(1);
		...
		{
			System.out.println("In HR.java in Performance method");
	...
	public float Par(String username)
	{
		float result=0.0f;
		try{
			int c=1;
			char dept = username.charAt(5);
			String[] compare= Performance(dept).split(",");
			
			Statement s1= c3.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			ResultSet r1=null;
			for(int i=1; i<11; i++)
			{
				int rmdone=0; int supdone=0;
				
				r1=s1.executeQuery("select Q"+i+" from REMARKS where MEMBER_ID='"+username+"' AND FLAG=2");
				r1.last();
				
				if(Integer.parseInt(r1.getString(1)) >= Integer.parseInt(compare[c]))
					supdone=1;
								
				r1.close();
				
				r1=s1.executeQuery("select Q"+i+" from REMARKS where MEMBER_ID='"+username+"' AND FLAG=3");
				r1.last();
				
				if( Integer.parseInt(r1.getString(1)) >= Integer.parseInt(compare[c+1]))
					rmdone=1;
				r1.close();
				
				c=c+2;
				if(supdone==1 && rmdone==1)
					result=result+1;
				else if(supdone==1 || rmdone==1)
					result=result+0.5f;
				}
			s1.close();
			}
		catch(Exception e){
			System.out.println("In HR.java in Par method");
			e.printStackTrace();
		}
		return result;
	}
